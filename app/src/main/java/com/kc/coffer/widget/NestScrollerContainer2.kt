...
import com.kc.coffer.widget.NestScrollerContainer2
 ...
 * @RevisionDescription：
 */
class NestScrollerContainer2 @JvmOverloads constructor(
    context: Context,
    ...
    override fun onStopNestedScroll(target: View, type: Int) {
        ...
        if (scrollY != MAX_HEIGHT) {
            //优化代码执行效率
//            animator!!.startOfFloat(target, scrollY.toFloat())
            // 优化代码执行的效率
            animator?.let {
                it.startOfFloat(target, scaleY)
            }
        }
    }
    ...
    override fun onNestedScroll(
        ...
        // 这个方法仅在目标View可以滚动时调用。
        Log.i(
            TAG, "onNestedScroll type : " + type + " ，dxConsumed ：" + dxConsumed + "，dyConsumed ："
        ...
        if (type == ViewCompat.TYPE_NON_TOUCH) {
            //非手指触发的滑动，即Filing
            //解决冗余fling问题
            if ((Math.floor(scrollY.toDouble()) == 0.0 || Math.ceil(scrollY.toDouble()) == (2 * MAX_HEIGHT).toDouble()) && !isFirstRunAnim) {
                var startY = 0
                if (dyUnconsumed > 0) {
    ...
    override fun onNestedPreScroll(target: View, dx: Int, dy: Int, consumed: IntArray, type: Int) {
        ...
        if (type == ViewCompat.TYPE_TOUCH) {
            Log.i(TAG, "onNestedPreScroll dy : $dy , getScrollY : $scrollY")
            ...
            val hiddenTop = dy > 0 && scrollY < MAX_HEIGHT && !target.canScrollVertically(-1)
            val showTop = dy < 0 && !target.canScrollVertically(-1)
            val hiddenBottom = dy < 0 && scrollY > MAX_HEIGHT && !target.canScrollVertically(1)
    ...
    private fun damping(dy: Int): Int {
        ...
        val i = (Math.abs(MAX_HEIGHT - scrollY) * 0.01).toInt()
     ...
     * 回弹动画
     */
    private inner class ReboundAnimator constructor() : ValueAnimator() {
    ...
    companion object {
        private const val TAG = "NestScroller_tag"
